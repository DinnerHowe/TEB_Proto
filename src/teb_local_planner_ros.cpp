...
TebLocalPlannerROS::TebLocalPlannerROS() : costmap_ros_(NULL),
                                           costmap_model_(NULL),
...
void TebLocalPlannerROS::initialize(std::string name,
...
bool TebLocalPlannerROS::computeVelocityCommands(geometry_msgs::Twist& cmd_vel) {
  ...
  nav_msgs::Odometry robot_pose_g = *(ros::topic::waitForMessage<nav_msgs::Odometry>("/odometry/filtered/global"));

  double dx = global_goal.getOrigin().getX() - robot_pose_g.pose.pose.position.x;
  double dy = global_goal.getOrigin().getY() - robot_pose_g.pose.pose.position.y;

  double delta_orient = g2o::normalize_theta( tf::getYaw(global_goal.getRotation()) - tf::getYaw(robot_pose_g.pose.pose.orientation));

  if(fabs(std::sqrt(dx*dx+dy*dy)) < 0.5  && fabs(delta_orient) < 0.1) {
    goal_reached_ = true;
    return true;
  }
  ...
  if (!feasible) {
    ...
    ROS_WARN("TebLocalPlannerROS stuck: trajectory is not feasible. Resetting planner...");
    ++no_infeasible_plans_; // increase number of infeasible solutions in a row
    time_last_infeasible_plan_ = ros::Time::now();
    last_cmd_ = cmd_vel;
    std_msgs::Bool msg;
    msg.data = true;
    ROS_INFO("Publish toopic on stuck!");
    return false;
  }

  // Get the velocity command for this sampling interval
  if (!planner_->getVelocityCommand(cmd_vel.linear.x,
                                    cmd_vel.linear.y,
                                    cmd_vel.angular.z)) {
    planner_->clearPlanner();
  ...
  saturateVelocity(cmd_vel.linear.x,
                   cmd_vel.linear.y,
                   cmd_vel.angular.z,
  ...
  if (cfg_.robot.cmd_angle_instead_rotvel) {
    cmd_vel.angular.z = convertTransRotVelToSteeringAngle(cmd_vel.linear.x,
                                                          cmd_vel.angular.z,
...
bool TebLocalPlannerROS::isGoalReached() {
  if (goal_reached_) {
    ...
    std_msgs::Bool msg;
    msg.data = true;
    ROS_INFO("Publish toopic on acheve_goal!");
    return true;
  }
  return false;
}
...
void TebLocalPlannerROS::updateObstacleContainerWithCostmapConverter() {
  ...
  for (std::size_t i=0; i<polygons->size(); ++i) {
    if (polygons->at(i).points.size()==1) { //  Point
      obstacles_.push_back(ObstaclePtr(new PointObstacle(polygons->at(i).points[0].x,
                                                         polygons->at(i).points[0].y)));
    } else if (polygons->at(i).points.size()==2) { //  Line
      obstacles_.push_back(ObstaclePtr(new LineObstacle(polygons->at(i).points[0].x,
                                                        polygons->at(i).points[0].y,
                                                        polygons->at(i).points[1].x,
                                                        polygons->at(i).points[1].y )));
    } else if (polygons->at(i).points.size()>2) { //  polygon
...
void TebLocalPlannerROS::updateObstacleContainerWithCustomObstacles() {
  ...
  if (!custom_obstacle_msg_.obstacles.empty()) {
    ...
    for (std::vector<geometry_msgs::PolygonStamped>::const_iterator obst_it = custom_obstacle_msg_.obstacles.begin();
...
Eigen::Vector2d TebLocalPlannerROS::tfPoseToEigenVector2dTransRot(const tf::Pose& tf_vel) {
  Eigen::Vector2d vel;
  vel.coeffRef(0) = std::sqrt(std::pow(tf_vel.getOrigin().getX(), 2) + std::pow(tf_vel.getOrigin().getY(), 2));
  vel.coeffRef(1) = tf::getYaw(tf_vel.getRotation());
  return vel;
}

bool TebLocalPlannerROS::pruneGlobalPlan(const tf::TransformListener& tf,
...
void TebLocalPlannerROS::saturateVelocity(double& vx,
                                          double& vy,
                                          double& omega,
                                          double max_vel_x,
                                          double max_vel_y,
  ...
  if (max_vel_x_backwards<=0) {
    ROS_WARN("TebLocalPlannerROS(): Do not choose max_vel_x_backwards to be <=0. "
             "Disable backwards driving by increasing the optimization weight "
             "for penalyzing backwards driving.");
  } else if (vx < -max_vel_x_backwards) {
    vx = -max_vel_x_backwards;
  }
}

double TebLocalPlannerROS::convertTransRotVelToSteeringAngle(double v,
                                                             double omega,
                                                             double wheelbase,
...
void TebLocalPlannerROS::validateFootprints(double opt_inscribed_radius,
    ...
    ROS_WARN_COND(opt_inscribed_radius + min_obst_dist < costmap_inscribed_radius,
                  "The inscribed radius of the footprint specified for TEB optimization (%f) "
                  "+ min_obstacle_dist (%f) are smaller "
...
void TebLocalPlannerROS::configureBackupModes(std::vector<geometry_msgs::PoseStamped>& transformed_plan,
  ...
  if (cfg_.recovery.shrink_horizon_backup &&
    ...
    (no_infeasible_plans_>0 || //  keep short horizon for at least a few seconds
    (current_time - time_last_infeasible_plan_).toSec() < cfg_.recovery.shrink_horizon_min_duration )) {
  ...
  if (cfg_.recovery.oscillation_recovery) {
    ...
    failure_detector_.update(last_cmd_,
                            cfg_.robot.max_vel_x,
                            cfg_.robot.max_vel_x_backwards,
                            max_vel_theta,
    ...
    // check if we have already detected an oscillation recently
...
RobotFootprintModelPtr TebLocalPlannerROS::getRobotFootprintFromParamServer(const ros::NodeHandle& nh) {
  ...
  if (model_name.compare("line") == 0) {
    ...
    ROS_INFO_STREAM("Footprint model 'line' (line_start: ["
                    << line_start[0] << "," << line_start[1] <<"]m, line_end: ["
